 ...
 *
 * Default sources (used when no --csv arguments given):
 ...
 *   php backend/scripts/seed-jyutping-from-multiple-csv.php
 *   php backend/scripts/seed-jyutping-from-multiple-csv.php --csv path/to/a.csv --csv path/to/b.csv
 *   php backend/scripts/seed-jyutping-from-multiple-csv.php --truncate  # clear table before import
 */
...
foreach ($csvs as $p) {
    ...
    if (!preg_match('/^\//', $p) && !preg_match('/^[A-Za-z]:\\\\/', $p)) {
        // relative path -> relative to project root script location
        $abs = $p;
    }
    if (!is_readable($abs)) {
...
function parse_tags($tags) {
    ...
    // Split on comma, semicolon, Chinese enumeration comma, whitespace
...
foreach ($validCsvs as $csvPath) {
    ...
    while (($row = fgetcsv($fp)) !== false) {
        ...
        // Validate UTF-8; if invalid, drop bad bytes but do not re-encode
        if ($hanzi !== '' && !mb_check_encoding($hanzi, 'UTF-8')) {
            $hanzi = iconv('UTF-8', 'UTF-8//IGNORE', $hanzi);
        }
        if ($word !== '' && !mb_check_encoding($word, 'UTF-8')) {
            $word = iconv('UTF-8', 'UTF-8//IGNORE', $word);
        }
        if ($tagsRaw !== '' && !mb_check_encoding($tagsRaw, 'UTF-8')) {
            $tagsRaw = iconv('UTF-8', 'UTF-8//IGNORE', $tagsRaw);
        }
        ...
        // Truncate hanzi to 10 characters (multibyte-safe)
        if ($hanzi !== '' && mb_strlen($hanzi, 'UTF-8') > 10) {
            $hanzi = mb_substr($hanzi, 0, 10, 'UTF-8');
        }
